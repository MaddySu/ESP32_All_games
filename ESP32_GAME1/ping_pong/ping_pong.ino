...
void waitForKeyPress() {
  while (true) {
    if (digitalRead(BUTTON1_PIN) == LOW || digitalRead(BUTTON3_PIN) == LOW ||
        digitalRead(BUTTON4_PIN) == LOW || digitalRead(BUTTON5_PIN) == LOW) {
      break;
    }
...
void loop() {
  ...
  if (gameRunning) {
    ...
    if (digitalRead(BUTTON3_PIN) == LOW && paddle1Y < display.height() - paddleHeight) {
      paddle1Y += 2;
    }
    if (digitalRead(BUTTON4_PIN) == LOW && paddle2Y > 0) {
      paddle2Y -= 2;
    }
    if (digitalRead(BUTTON5_PIN) == LOW && paddle2Y < display.height() - paddleHeight) {
      paddle2Y += 2;
    }
